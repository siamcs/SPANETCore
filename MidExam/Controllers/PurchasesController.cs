...
namespace MidExam.Controllers
{
    public class PurchasesController : Controller
    {
        ...
        public PurchasesController(ApplicationDbContext _db)
        {
            db = _db;
        }
        ...
        {
            ViewData["MeasurementUnitOptions"] = new SelectList(db.MeasurementUnits, "Id", "Name");
            ViewData["ProductOptions"] = new SelectList(db.Products, "Id", "Name");

            var purchasesWithDetails = await db.PurchaseHeaders.Include(p => p.purchaseDetails).ThenInclude(p => p.MeasurementUnit).ToListAsync();
        ...
        public async Task<IActionResult> Create(PurchaseHeaderVM purchaseHeaderVM, string token)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    await db.PurchaseHeaders.AddAsync(purchaseHeader);
                    await db.SaveChangesAsync();

                    var products = await db.Products.ToListAsync();
                    var measurementUnits = await db.MeasurementUnits.ToListAsync();
        ...
        public async Task<IActionResult> CreateExecutingSP(PurchaseHeaderVM purchaseHeaderVM, string token)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    await db.Database.ExecuteSqlAsync($"EXEC dbo.PurchaseSP {customerName}, {customerPhoneNumber}, {customerEmailAddress}, {invoiceNumber}, {purchaseDate}, {tvp}");
